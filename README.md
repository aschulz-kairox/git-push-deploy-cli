...
│         │                                   └────────┬────────┘            │
│         │                                            │                      │
│         │ 5. Send notification                       │ 3. App restarted    │
│         │    to Slack/Discord                        ▼                      │
│         │                                   ┌─────────────────┐            │
│         ▼                                   │  App Running    │            │
│   ┌─────────────┐                           │  (zero-downtime)│            │
...
│   └─────────────┘                                                          │
│         │                                                                   │
│         └── Developer receives: "✅ my-api deployed successfully"          │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
```
┌─────────────────────────────────────────────────────────────────────────────┐
│                     GPD JANUS ARCHITECTURE                                   │
│                  (Same CLI, Two Roles)                                       │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│   ┌─────────────────────────────────┐ ┌─────────────────────────────────┐  │
│   │      LOCAL (Dev Machine)        │ │      SERVER (Production)        │  │
│   │      ════════════════════       │ │      ════════════════════       │  │
│   │                                 │ │                                 │  │
│   │   $ gpd deploy                  │ │   $ gpd install                 │  │
│   │   $ gpd init                    │ │     (called by git hook)        │  │
│   │   $ gpd stage                   │ │                                 │  │
│   │   $ gpd release                 │ │                                 │  │
│   │   $ gpd rollback                │ │                                 │  │
│   │   $ gpd config                  │ │                                 │  │
...
│                  │                                   ▲                      │
│                  │                                   │                      │
│                  │ 1. git push                       │ 2. post-receive      │
│                  │    (artifacts)                    │    hook calls        │
│                  │                                   │    gpd install       │
│                  ▼                                   │                      │
│   ┌─────────────────────────────────────────────────────────────────────┐  │
│   │                                                                     │  │
│   │   #!/bin/bash                                                       │  │
│   │   # post-receive hook (generated by gpd init)                       │  │
│   │                                                                     │  │
│   │   gpd install my-service -c /opt/app/.git-deploy.json              │  │
│   │   ▲                                                                 │  │
│   │   │                                                                 │  │
│   │   └── GPD calls ITSELF on the server!                               │  │
│   │                                                                     │  │
│   └─────────────────────────────────────────────────────────────────────┘  │
│                                                                             │
│   Installation:                                                             │
│   ─────────────                                                             │
│                                                                             │
│   # On dev machine                      # On server                         │
│   npm install -g git-push-deploy-cli    npm install -g git-push-deploy-cli │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```
